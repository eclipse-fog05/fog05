...
class RuntimePluginFDU(Plugin):
    ...
    def stop_fdu(self, fdu_uuid):
        ...
        if the fdu state do not allow transition to CONFIGURED (in this case is not in RUNNING)
        should throw an exception
    ...
    def migrate_fdu(self, fdu_uuid, dst=False):
        '''
        Migrate the fdu identified by fdu_uuid to the new FogNode identified by fognode_uuid
        The migration depend to the nature of the fdu (native app, µSvc, VM, Container)
        if the fdu state do not allow transition to MIGRATE (eg a native app can't be migrated)
        should throw an exception
    ...
    def scale_fdu(self, fdu_uuid):
        ...
        if the fdu state do not allow transition to SCALE (in this case is not in RUNNING)
        should throw an exception
    ...
    def pause_fdu(self, fdu_uuid):
        ...
        if the fdu state do not allow transition to PAUSED (in this case is not in RUNNING)
        should throw an exception
    ...
    def resume_fdu(self, fdu_uuid):
        ...
        if the fdu state do not allow transition to RUNNING (in this case is not in PAUSED)
        should throw an exception
    ...
    def configure_fdu(self, fdu_uuid):
        ...
        This should create an fdu instance that can be addressed by .../fdu/<fdu_uuid>/instance/<instance_uuid>
        not clear at the time if instance uuid should be generated by this method or came as a paramenter
        This state is the separation from fdu and fdu instances, because here we create the files

        if the fdu state do not allow transition to CONFIGURED (in this case is not in DEFINED)
        should throw an exception
    ...
    def clean_fdu(self, fdu_uuid):
        ...
        if the fdu state do not allow transition to DEFINED (in this case is not in CONFIGURED)
        should throw an exception
