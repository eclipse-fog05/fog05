...
class GAD(object):
    ...
    def get_all_atomic_entities(self, sysid, tenantid):
        s = self.get_all_atomic_entity_selector(sysid, tenantid)
        res = self.ws.get(s)
        if len(res) == 0:
            return []
        else:
    ...
    def get_atomic_entity_info(self, sysid, tenantid, aeid):
        p = self.get_atomic_entity_info_path(sysid, tenantid, aeid)
        res = self.ws.get(p)
        if len(res) == 0:
            return None
        v = res[0][1]
    ...
    def add_atomic_entity_info(self, sysid, tenantid, aeid, aeinfo):
        p = self.get_atomic_entity_info_path(sysid, tenantid, aeid)
        v = Value(json.dumps(aeinfo), encoding=Encoding.STRING)
        return self.ws.put(p, v)

    def remove_atomic_entity_info(self, sysid, tenantid, aeid):
        p = self.get_atomic_entity_info_path(sysid, tenantid, aeid)
        self.ws.remove(p)

    def observe_atomic_entities(self, sysid, tenantid, callback):
        s = self.get_all_atomic_entity_selector(sysid, tenantid)
        def cb(kvs):
            if len(kvs) == 0:
                raise ValueError('Listener received empty data')
