...
let plugin_list nodeid yserver = 
  match nodeid with
  | Some node_uuid ->
    Cli_helper.get_all_node_plugin node_uuid yserver >>= 
    fun d -> Cli_printing.print_node_plugin d 
  | None -> Lwt_io.printf "Node uuid parameter missing!!\n"

let plugin_add nodeid manifest yserver = 
  match nodeid with
  | Some node_uuid ->
    (match manifest with
     | Some path ->  
...
let plugin_cmd action nodeid _ manifest =
  match action with
  | "list" -> plugin_list nodeid Cli_helper.yaksserver
  | "add" -> plugin_add nodeid manifest Cli_helper.yaksserver
...
let node_list () = 
  ...
  let _ = Lwt_io.printf "+---------------------------------------------+\n" in
  Cli_helper.get_all_nodes Cli_helper.yaksserver >>=
  Lwt_list.iter_p (
    fun (_,n) -> 
...
let node_info nodeid = 
  ...
  | Some node_uuid -> 
    ...
    let%lwt s = Store.create (Printf.sprintf "a%s" Cli_helper.home) Cli_helper.aroot Cli_helper.ahome Cli_helper.yaksserver in
     ...
     | Some path ->  
       ...
       (match res with
        | Ok _ ->  
          ...
          Cli_helper.send_add_network_node manifest id Cli_helper.yaksserver >>= fun _ ->
...
let network_remove nodeid netid =
  ...
  | Some node_uuid ->
    (match netid with
     | Some net_uuid -> 
       Cli_helper.send_remove_network_node net_uuid node_uuid Cli_helper.yaksserver >>= fun _ ->
       Lwt_io.printf "Removed network %s from node  %s\n" net_uuid node_uuid
     | None -> Lwt_io.printf "Atomic Entity uuid parameter missing!!\n")
  | None ->  Lwt_io.printf "Node uuid parameter missing!!\n"

let network_list () = 
  Cli_helper.get_all_networks Cli_helper.yaksserver >>= Cli_printing.print_networks
...
let add_entity manifest = 
  match manifest with
  | Some path -> 
    Cli_helper.onboard path Cli_helper.yaksserver
  | None -> Lwt_io.printf "Manifest parameter missing!!\n"

let remove_entity entityid = 
  match entityid with
  | Some id ->
    Cli_helper.offload id Cli_helper.yaksserver
     ...
     | Some path ->  
       ...
       (match res with
        | Ok _ ->  
          ...
          Cli_helper.send_atomic_entity_define manifest id Cli_helper.yaksserver >>= fun _ ->
          Lwt_io.printf "Atomic Entity Defined\n";
        | Error e -> Lwt_io.printf "Manifest has errors: %s\n" (Printexc.to_string e))
     |None  -> Lwt_io.printf "Manifest parameter missing!!\n")
  | None ->  Lwt_io.printf "Node uuid parameter missing!!\n"
  ...
  | Some node_uuid ->
    (match entityid with
     | Some entity_uuid -> 
       ...
       let%lwt _ = Cli_helper.send_atomic_entity_remove node_uuid entity_uuid Cli_helper.yaksserver in
     ...
     | Some entity_uuid ->
       (match instanceid with 
        | Some instance_uuid -> 
          ...
          let%lwt _ = Cli_helper.send_atomic_entity_instance_remove node_uuid entity_uuid instance_uuid Cli_helper.yaksserver in
          Lwt_io.printf "Atomic Entity Cleaned\n";
        | None -> Lwt_io.printf "Instance uuid parameter missing!!\n")
     | None -> Lwt_io.printf "Atomic Entity uuid parameter missing!!\n")
  | None ->  Lwt_io.printf "Node uuid parameter missing!!\n"

let atomic_entity_change_state nodeid entityid instanceid action newstate  = 
  match nodeid with
  | Some node_uuid ->
    (match entityid with
     | Some entity_uuid ->
       (match instanceid with 
        | Some instance_uuid -> 
          ...
          let%lwt _ = Cli_helper.send_atomic_entity_instance_action node_uuid entity_uuid instance_uuid action newstate Cli_helper.yaksserver in
        ...
        | Some instance_uuid -> 
          (match destinationid with 
           |Some destination_node ->  
             Cli_helper.send_migrate_atomic_entity_instance node_uuid entity_uuid instance_uuid destination_node Cli_helper.yaksserver
...
let atomic_entity_list () = 
  Cli_helper.get_all_atomic_entities Cli_helper.yaksserver >>= Cli_printing.print_atomic_entities

let entity_list () = 
  Cli_helper.get_all_entities Cli_helper.yaksserver >>= Cli_printing.print_entities
