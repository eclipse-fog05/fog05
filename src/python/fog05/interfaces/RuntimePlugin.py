...
class RuntimePlugin(Plugin):
    ...
    def run_entity(self, enitity_uuid, instance_uuid=None):
        ...
        if the entity state do not allow transition to RUN
        (eg is non CONFIGURED) should throw an exception
    ...
    def stop_entity(self, enitity_uuid, instance_uuid=None):
        ...
        if the entity state do not allow transition to CONFIGURED
        (in this case is not in RUNNING)should throw an exception
    ...
    def migrate_entity(self, entity_uuid, dst=False, instance_uuid=None):
        '''
        Migrate the entity identified by entity_uuid to the new FogNode
         identified by fognode_uuid
        The migration depend to the nature of the entity
         (native app, µSvc, VM, Container)
        if the entity state do not allow transition to MIGRATE
         (eg a native app can't be migrated)
        should throw an exception
        if the destination node can't handle the migration an exception
         should be throwed
    ...
    def scale_entity(self, entity_uuid, instance_uuid=None):
        '''
        Scale an entity
        eg. give more cpu/ram/disk,
         maybe passed by parameter the new scale value?

        if the entity state do not allow transition to SCALE
         (in this case is not in RUNNING) should throw an exception
    ...
    def pause_entity(self, entity_uuid, instance_uuid=None):
        ...
        if the entity state do not allow transition to
        PAUSED (in this case is not in RUNNING)
        should throw an exception
    ...
    def configure_entity(self, entity_uuid, instance_uuid=None):
        ...
        This should create an entity instance that can be addressed by
        .../entity/<entity_uuid>/instance/<instance_uuid>
        not clear at the time if instance uuid should be generated by
        this method or came as a paramenter
        This state is the separation from entity and entity instances,
         because here we create the files

        if the entity state do not allow transition to
        CONFIGURED (in this case is not in DEFINED)
        should throw an exception
    ...
    def clean_entity(self, entity_uuid, instance_uuid=None):
        ...
        if the entity state do not allow transition to
        DEFINED (in this case is not in CONFIGURED)
        should throw an exception
