...
use fog05_sdk::types::{VirtualInterface, VirtualNetwork,
    Interface, IPAddress, MACAddress, ConnectionPoint,
    NetworkNamespace, VirtualInterfaceConfig, PluginKind, LinkKind,
    MCastVXLANInfo, IPVersion, IPConfiguration, VirtualInterfaceConfigKind,
    VirtualInterfaceKind, VETHKind, VLANKind, VXLANKind, BridgeKind,
    GREKind, MACVLANKind, InterfaceKind};
...
impl NetworkingPlugin for DummyNetwork {
    ...
    async fn create_default_virtual_network(&self, dhcp : bool) -> FResult<VirtualNetwork> {
        ...
        self.connector.global.add_node_virutal_network(node_uuid, default_vnet.clone()).await?;
        Ok(default_vnet)
    }
    ...
    async fn create_virtual_network(&self, vnet_uuid : Uuid) -> FResult<VirtualNetwork> {
        ...
        match self.connector.global.get_virtual_network(vnet_uuid).await {
            Ok(vnet) => {
                let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
                self.connector.global.add_node_virutal_network(node_uuid, vnet.clone()).await?;
                Ok(vnet)
            },
            Err(FError::NotFound) => {
    ...
    async fn get_virtual_network(&self, vnet_uuid : Uuid) -> FResult<VirtualNetwork> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        self.connector.global.get_node_virtual_network(node_uuid, vnet_uuid).await
    }

    async fn delete_virtual_network(&self, vnet_uuid : Uuid) -> FResult<VirtualNetwork> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        match self.connector.global.get_node_virtual_network(node_uuid, vnet_uuid).await {
            Err(_) => Err(FError::NotFound),
            Ok(vnet) => {
                self.connector.global.remove_node_virtual_network(node_uuid, vnet_uuid).await?;
                Ok(vnet)
            }
        }
    }
    ...
    async fn create_connection_point(&self) -> FResult<ConnectionPoint> {
        ...
        match self.connector.global.get_node_connection_point(node_uuid, cp_uuid).await {
            Err(_) => {
                ...
                self.connector.global.add_node_connection_point(node_uuid, cp.clone()).await?;
                Ok(cp)
            }
    ...
    async fn get_connection_point(&self, cp_uuid : Uuid) -> FResult<ConnectionPoint> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        self.connector.global.get_node_connection_point(node_uuid, cp_uuid).await
    }
    ...
    async fn delete_connection_point(&self, cp_uuid : Uuid) -> FResult<Uuid> {
        ...
        match self.connector.global.get_node_connection_point(node_uuid, cp_uuid).await {
            Err(_) => Err(FError::NotFound),
            Ok(_) => {
                self.connector.global.remove_node_connection_point(node_uuid, cp_uuid).await?;
                Ok(cp_uuid)
            }
        }
    }
        ...
        match intf.kind {
            VirtualInterfaceConfigKind::VXLAN(conf) => {
                let v_iface = VirtualInterface {
                    ...
                    net_ns : None,
                    parent : None,
                    kind : VirtualInterfaceKind::VXLAN(
                            VXLANKind {
                                vni : conf.vni,
                ...
                self.connector.global.add_node_interface(node_uuid, v_iface.clone()).await?;
                Ok(v_iface)
            }
            VirtualInterfaceConfigKind::BRIDGE => {
                let v_iface = VirtualInterface {
                    ...
                    net_ns : None,
                    parent : None,
                    kind : VirtualInterfaceKind::BRIDGE(
                            BridgeKind {
                                childs : Vec::new(),
                            }
                    ),
            ...
            VirtualInterfaceConfigKind::VETH => {
                ...
                self.connector.global.add_node_interface(node_uuid, v_iface_internal.clone()).await?;
                self.connector.global.add_node_interface(node_uuid, v_iface_external).await?;
                Ok(v_iface_internal)
            }
            VirtualInterfaceConfigKind::VLAN(conf) => {
                let v_iface = VirtualInterface {
                    ...
                    net_ns : None,
                    parent : None,
                    kind : VirtualInterfaceKind::VLAN(
                            VLANKind {
                                tag : conf.tag,
                                dev : self.get_dummy_face(),
                            }
            ...
            VirtualInterfaceConfigKind::MACVLAN => {
                let v_iface = VirtualInterface {
                    ...
                    net_ns : None,
                    parent : None,
                    kind : VirtualInterfaceKind::MACVLAN(
                            MACVLANKind {
                                dev : self.get_dummy_face(),
                            }
                    ),
            ...
            VirtualInterfaceConfigKind::GRETAP(conf)  => {
                let v_iface = VirtualInterface {
                    ...
                    net_ns : None,
                    parent : None,
                    kind : VirtualInterfaceKind::GRETAP(
                            GREKind {
                                local_addr : conf.local_addr,
                                remote_addr : conf.remote_addr,
                                ttl : conf.ttl,
            ...
            VirtualInterfaceConfigKind::IP6GRE(conf)  => {
                let v_iface = VirtualInterface {
                    ...
                    net_ns : None,
                    parent : None,
                    kind : VirtualInterfaceKind::IP6GRE(
                            GREKind {
                                local_addr : conf.local_addr,
                                remote_addr : conf.remote_addr,
                                ttl : conf.ttl,
            ...
            VirtualInterfaceConfigKind::IP6GRETAP(conf)  => {
                let v_iface = VirtualInterface {
                    ...
                    net_ns : None,
                    parent : None,
                    kind : VirtualInterfaceKind::IP6GRETAP(
                            GREKind {
                                local_addr : conf.local_addr,
                                remote_addr : conf.remote_addr,
                                ttl : conf.ttl,
    ...
    async fn get_virtual_interface(&self, intf_uuid : Uuid) -> FResult<VirtualInterface> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        self.connector.global.get_node_interface(node_uuid, intf_uuid).await
    }

    async fn delete_virtual_interface(&self, intf_uuid : Uuid) -> FResult<VirtualInterface> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        match self.connector.global.get_node_interface(node_uuid, intf_uuid).await {
            Err(_) => Err(FError::NotFound),
            Ok(intf) => {
                self.connector.global.remove_node_interface(node_uuid, intf_uuid).await?;
                Ok(intf)
            }
        }
    }
    ...
    async fn get_virtual_bridge(&self, br_uuid : Uuid) -> FResult<VirtualInterface> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        match self.connector.global.get_node_interface(node_uuid, br_uuid).await {
            Err(err) => Err(err),
            Ok(i) => {
                match i.kind {
            ...
            Ok(i) => {
                match i.kind {
                    VirtualInterfaceKind::BRIDGE(_) => {
                        self.connector.global.remove_node_interface(node_uuid, br_uuid).await?;
                        Ok(i)
                    },
                    _ => Err(FError::WrongKind),
                }
    ...
    async fn create_network_namespace(&self) -> FResult<NetworkNamespace> {
        ...
        self.connector.global.add_node_network_namespace(node_uuid, netns.clone()).await?;
        Ok(netns)
    }

    async fn get_network_namespace(&self, ns_uuid : Uuid) -> FResult<NetworkNamespace> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        self.connector.global.get_node_network_namespace(node_uuid, ns_uuid).await
    }

    async fn delete_network_namespace(&self, ns_uuid : Uuid) -> FResult<NetworkNamespace> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        match self.connector.global.get_node_network_namespace(node_uuid, ns_uuid).await {
            Err(_) => Err(FError::NotFound),
            Ok(netns) => {
                self.connector.global.remove_node_network_namespace(node_uuid, ns_uuid).await?;
                Ok(netns)
            }
        }
    }
    ...
    async fn bind_interface_to_connection_point(&self, intf_uuid : Uuid, cp_uuid : Uuid) -> FResult<VirtualInterface> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        let cp = self.connector.global.get_node_connection_point(node_uuid, cp_uuid).await?;
        let mut iface = self.connector.global.get_node_interface(node_uuid, intf_uuid).await?;
        iface.net_ns = Some(cp.net_ns);
        self.connector.global.add_node_interface(node_uuid, iface.clone()).await?;
        Ok(iface)
    ...
    async fn bind_connection_point_to_virtual_network(&self, cp_uuid : Uuid, vnet_uuid : Uuid) -> FResult<ConnectionPoint> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        let cp = self.connector.global.get_node_connection_point(node_uuid, cp_uuid).await?;
        let mut vnet = self.connector.global.get_node_virtual_network(node_uuid, vnet_uuid).await?;
    ...
    async fn get_interface_addresses(&self, intf_uuid : Uuid) -> FResult<Vec<IPAddress>> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        let iface = self.connector.global.get_node_interface(node_uuid, intf_uuid).await?;
        Ok(iface.addresses)
    }
    ...
    async fn move_interface_info_namespace(&self, intf_uuid : Uuid, ns_uuid : Uuid) -> FResult<VirtualInterface> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        let mut iface = self.connector.global.get_node_interface(node_uuid, intf_uuid).await?;
        let mut netns = self.connector.global.get_node_network_namespace(node_uuid, ns_uuid).await?;
    ...
    async fn move_interface_into_default_namespace(&self, intf_uuid : Uuid) -> FResult<VirtualInterface> {
        ...
        match iface.net_ns {
            Some(netns_uuid) => {
                iface.net_ns = None;
                self.connector.global.add_node_interface(node_uuid, iface.clone()).await?;
                let mut netns = self.connector.global.get_node_network_namespace(node_uuid, netns_uuid).await?;
    ...
    async fn attach_interface_to_bridge(&self, intf_uuid : Uuid, br_uuid : Uuid) -> FResult<VirtualInterface> {
        let node_uuid = self.agent.as_ref().unwrap().get_node_uuid().await??;
        let mut iface = self.connector.global.get_node_interface(node_uuid, intf_uuid).await?;
        let bridge = self.connector.global.get_node_interface(node_uuid, br_uuid).await?;
        match bridge.kind {
            VirtualInterfaceKind::BRIDGE(mut info) => {
                iface.parent = Some(bridge.uuid);
                info.childs.push(iface.uuid);
                let mut new_bridge = self.connector.global.get_node_interface(node_uuid, br_uuid).await?;
                new_bridge.kind = VirtualInterfaceKind::BRIDGE(info);
                self.connector.global.add_node_interface(node_uuid, iface.clone()).await?;
                self.connector.global.add_node_interface(node_uuid, new_bridge.clone()).await?;
                Ok(iface)
            }
            _ => Err(FError::WrongKind)
        }
...
impl DummyNetwork {
    async fn run(&self, stop : async_std::sync::Receiver<()>) {
        ...
        self.agent.clone().unwrap().register_plugin(self.uuid, PluginKind::NETWORKING).await.unwrap().unwrap();
