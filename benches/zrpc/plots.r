...
raw_data <- bind_rows(get_data, call_data, state_data, eval_data, zstate_data) %>% mutate(MBIT_THR = THR/1024/1024)
...
#   ggtitle("all primitives msg/s localhost") +
...
#   scale_x_discrete(breaks = sizes, labels = sizes) +
#   ggtitle("all primitives throughput localhost") +
...
#   scale_y_log10() +
#   scale_x_discrete(breaks = sizes, labels = sizes) +
#   ggtitle("all primitives rtt localhost") +
#   xlab("Payload size") + ylab("RTT µS")
# plot(p_rtt)

#ggsave("comparison.pdf",plot = p_msgs, width = 10, height = 10, limitsize = TRUE);
#ggsave("comparison.pdf",plot = p_thr, width = 10, height = 10, limitsize = TRUE);
